...
export default function MealPlannerWorldClass() {
  ...
  // Read localStorage lazily once on mount
  const [storedData] = useState(() => {
    try {
      const item = localStorage.getItem("mealPlanned");
  ...
  const hasUrlParams = Boolean(searchParams.get("budget"));
  const hasSavedPlan = Boolean(
    storedData && Array.isArray(storedData.plan) && storedData.plan.length > 0
  );

  const budget = Number(searchParams.get("budget")) || Number(storedData?.budget) || 0;
  const mealPeople = Number(searchParams.get("people")) || Number(storedData?.mealPeople) || 1;
  const mealNumber = searchParams.get("meals") || storedData?.mealNumber || "3";
  const diet = searchParams.get("diet") || storedData?.diet || "mixed";

  const [generatedMeals, setGenerateMeals] = useState(() => {
    if (hasSavedPlan) return storedData.plan;
    return [];
  });
  ...
  const getMealCost = useCallback(
    (item) => (item ? item.costEstimate * (mealPeople / item.servings) : 0),
    [mealPeople]
  );

  const getMealCalories = useCallback(
    (item) =>
      item ? Math.round(Number(item.calories || 0) * (mealPeople / (item.servings || 1))) : 0,
    [mealPeople]
  );

  const runMealGeneration = useCallback(
    (selectedDiet) => {
      if (!allRecipes || allRecipes.length === 0) return [];
      setBudgetError(false);

      let filteredDiet = allRecipes;
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          for (let j = 0; j < foodNames.length; j++) {
            let mealBudget = DailyBudget;
            if (foodNames.length === 3) {
              mealBudget = Math.floor(DailyBudget * (j === 2 ? 0.4 : 0.3));
            } else if (foodNames.length === 2) {
              mealBudget = Math.floor(DailyBudget * (j === 0 ? 0.4 : 0.6));
            }

            const budgetSafety = filteredDiet.filter(
              (item) => getMealCost(item) <= mealBudget
            );
            ...
            const chosenMeal = budgetSafety[randomIndex];
            recipeStore.push(chosenMeal);
            totalPlanCost += getMealCost(chosenMeal);
          }
          allRecipeStored.push(recipeStore);
        }

        if (budget === 0 || totalPlanCost <= budget) {
    ...
    [allRecipes, foodNames, DailyBudget, budget, getMealCost]
  );

  // Sync generation on search parameter change
  useEffect(() => {
    if (!allRecipes || allRecipes.length === 0) return;

    const currentBudgetParam = searchParams.get("budget");
    const currentPeopleParam = searchParams.get("people");
    const currentMealsParam = searchParams.get("meals");
    const currentDietParam = searchParams.get("diet");

    const settingsChanged =
      (currentBudgetParam && String(currentBudgetParam) !== String(storedData?.budget)) ||
      (currentPeopleParam && String(currentPeopleParam) !== String(storedData?.mealPeople)) ||
      (currentMealsParam && String(currentMealsParam) !== String(storedData?.mealNumber)) ||
      (currentDietParam && String(currentDietParam) !== String(storedData?.diet));

    if ((generatedMeals.length === 0 || settingsChanged) && budget > 0) {
    ...
    budget,
    storedData?.budget,
    storedData?.mealPeople,
    storedData?.mealNumber,
    storedData?.diet,
  ]);

  // Sync to local storage when state updates
  useEffect(() => {
    if (generatedMeals.length > 0 && budget > 0) {
  ...
  const totalCost = generatedMeals[selectedDay]?.reduce((acc, mov) => acc + getMealCost(mov), 0) ?? 0;
  const totalCalories = generatedMeals[selectedDay]?.reduce((acc, mov) => acc + getMealCalories(mov), 0) ?? 0;
  const totalWeeklyCost = generatedMeals.flat().reduce((acc, mov) => acc + getMealCost(mov), 0) ?? 0;

  const totalWeeklyCost2 = Math.round(totalWeeklyCost);
  const totalCost2 = Math.round(totalCost);
  const foodPercentage = budget ? Math.round((totalWeeklyCost / budget) * 100) : 0;

  const shouldShowModal = isEditing || (!hasUrlParams && !hasSavedPlan);

  // Opens budget edit modal & clears the missing identifier error + unused setIsEditing warning
...
return (
  ...
  <ModalPlanner 
    ...
    onPlanSubmit={() => {
      setIsEditing(false);
      setGenerateMeals([]); // Clear current meals to force regenerate
    }} 
  />
) : (
      <div className="min-h-screen w-full overflow-x-hidden bg-[radial-gradient(circle_at_top_left,rgba(255,116,23,.20),transparent_30%),linear-gradient(135deg,#fff7ed_0%,#ffe9d1_46%,#fffaf5_100%)] text-[#33211b] antialiased selection:bg-orange-200 selection:text-[#33211b]">
        <main className="mx-auto min-h-screen w-full">
          <section className="border border-white/70 bg-[#fff8ef]/95 p-4 shadow-[0_18px_55px_rgba(112,66,31,.13)] backdrop-blur-xl transition duration-500 sm:p-6 lg:p-8">
            ...
            <header className="flex flex-wrap items-center justify-between gap-4 border-b border-orange-100 pb-5">
              ...
              <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-row">
                <button className="group flex min-h-[52px] w-full items-center justify-center gap-2 rounded-xl border border-orange-100 bg-white px-3 py-2 text-[13px] font-extrabold leading-tight text-[#33211b] shadow-[0_8px_24px_rgba(88,54,28,.08)] transition duration-300 hover:-translate-y-0.5 hover:border-orange-200 hover:bg-orange-50 focus-visible:ring-4 focus-visible:ring-orange-200 sm:h-11 sm:w-auto sm:px-4 sm:text-sm">
                ...
                <button className="group flex min-h-[52px] w-full items-center justify-center gap-2 rounded-xl bg-[#ff7417] px-3 py-2 text-[13px] font-extrabold leading-tight text-white shadow-[0_10px_28px_rgba(255,116,23,.22)] transition duration-300 hover:-translate-y-0.5 hover:bg-[#e85f08] focus-visible:ring-4 focus-visible:ring-orange-200 sm:h-11 sm:w-auto sm:px-4 sm:text-sm">
            ...
            {/* Budget Analytics & Day Overview */}
            <section className="mt-5 grid items-center gap-5 lg:grid-cols-[1.5fr_.5fr]">
              {/* Budget Overview Card */}
              <div className="overflow-hidden rounded-[28px] border border-[#f5d9bf] bg-[#fffaf5] p-4 shadow-[0_10px_35px_rgba(88,54,28,.07)] sm:p-5 lg:p-6">
                <div className="rounded-[24px] border border-orange-100 bg-white/85 p-5 shadow-[0_18px_50px_rgba(120,72,32,0.10)] backdrop-blur sm:p-6">
                  <div className="grid gap-5 sm:grid-cols-3 lg:grid-cols-[1.45fr_1fr_1fr_auto] lg:items-center lg:gap-6">
                    ...
                    <div className="rounded-2xl bg-emerald-50/70 p-4 ring-1 ring-emerald-100 sm:p-5 lg:bg-transparent lg:p-0 lg:ring-0">
                      <p className="flex items-center gap-2 text-[12px] font-black uppercase tracking-[0.14em] text-[#7a604f]">
                        <span className={`size-2.5 rounded-full ${budget - totalWeeklyCost2 < 0 ? "bg-red-500 shadow-[0_0_0_5px_rgba(239,68,68,.12)]" : "bg-emerald-600 shadow-[0_0_0_5px_rgba(22,163,74,.12)]"}`} />
                        Remaining
                      </p>
                      <p className={`mt-2 text-2xl font-black tracking-[-0.03em] sm:text-[28px] ${budget - totalWeeklyCost2 < 0 ? "text-red-500" : "text-emerald-600"}`}>
                    ...
                    <div className="flex items-center justify-start rounded-2xl bg-white/70 p-4 ring-1 ring-orange-100/70 sm:col-span-3 lg:col-span-1 lg:justify-end lg:bg-transparent lg:p-0 lg:ring-0">
                      <button
                        ...
                        className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm transition-all duration-200 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-900 active:scale-95"
                      >
                    ...
                    <div className="mt-3 h-3 overflow-hidden rounded-full bg-[#ffe5ca] ring-1 ring-orange-100">
                      <div
                        className={`h-full rounded-full transition-all duration-700 ${
                          foodPercentage > 100
                            ? "bg-red-500 shadow-[0_10px_24px_rgba(239,68,68,.24)]"
                            : "bg-gradient-to-r from-[#ff8a22] to-[#ff6414] shadow-[0_10px_24px_rgba(255,116,23,.24)]"
                        }`}
                  ...
                  <div className={`mt-4 flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold ${
                    foodPercentage > 100 ? "bg-red-50 text-red-600" : "bg-[#f7fff7] text-green-600"
                  }`}>
                    <span className={`grid size-5 shrink-0 place-items-center rounded-full border ${
                      foodPercentage > 100 ? "border-red-200 bg-white text-red-500" : "border-green-200 bg-white text-green-600"
                    }`}>
                    ...
                    <span className="leading-relaxed">
                      {foodPercentage > 100
                        ? "You've exceeded your planned weekly budget!"
            ...
            <nav className="mt-5 flex items-center gap-3 overflow-x-auto rounded-2xl border border-orange-100 bg-white/80 p-3 shadow-[0_8px_24px_rgba(88,54,28,.08)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {weekData.map((day, index) => {
                const active = index === selectedDay ? "bg-orange-500 text-white" : "bg-[#fff8ef] text-[#33211b]";
                      ...
                      <article key={index} className="group grid gap-3 rounded-2xl border border-orange-100 bg-white p-3 shadow-[0_8px_24px_rgba(88,54,28,.08)] transition duration-300 hover:-translate-y-1 hover:border-orange-200 hover:shadow-[0_18px_55px_rgba(112,66,31,.13)] sm:grid-cols-[112px_1fr_auto] sm:items-center lg:grid-cols-[140px_1fr_auto]">
                        ...
                        <div className="grid gap-3 min-[520px]:grid-cols-[112px_1fr] min-[520px]:items-center">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm font-semibold text-[#8a5c45]">
                              {getMealCalories(mealItem)} cal · {mealPeople} {mealPeople === 1 ? "serving" : "servings"}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end">
                          <p className="text-sm font-black text-[#ff7417]">
                            ₦{mealItem ? Math.round(getMealCost(mealItem)).toLocaleString() : 0}
                          </p>
                          <div className="flex gap-2">
                            <button
                              onClick={() => {
                                let currentMealSlotBudget = 0;
                                if (foodNames.length === 3) {
                                  currentMealSlotBudget = (index === 0 || index === 1)
                                    ? Math.floor(DailyBudget * 0.3)
                                    : Math.floor(DailyBudget * 0.4);
                                } else if (foodNames.length === 2) {
                                  currentMealSlotBudget = (index === 0)
                                    ? Math.floor(DailyBudget * 0.4)
                                    : Math.floor(DailyBudget * 0.6);
                                } else if (foodNames.length === 1) {
                                  currentMealSlotBudget = DailyBudget;
                                }

                                const currentMealCost = getMealCost(mealItem);
                                const availableBudgetForSwap = (budget - totalWeeklyCost2) + currentMealCost;

                                const filteredResult = allRecipes.filter((item) => {
                                  const itemCost = getMealCost(item);
                                  const matchesDiet =
                                    diet === "nigerian" ? item.category === "Nigerian" :
                                    diet === "international" ? item.category === "International" : true;

                                  const matchesSlotBudget = itemCost <= currentMealSlotBudget;
                                  const matchesWeeklyCap = budget === 0 || itemCost <= availableBudgetForSwap;

                                  return matchesDiet && matchesSlotBudget && matchesWeeklyCap;
                                });
                              ...
                              className="grid place-items-center rounded-lg border border-orange-100 px-2.5 py-1 text-xs font-bold text-[#8a5c45] transition hover:bg-orange-50 active:scale-95"
                            >
                              Swap
                            </button>
                            <button
                              onClick={() => handleDeleteMeal(selectedDay, index)}
                              className="grid place-items-center rounded-lg border border-red-100 px-2.5 py-1 text-xs font-bold text-red-500 transition hover:bg-red-50 active:scale-95"
                            >
                              Delete
                            </button>
                          </div>
          ...
          <button
            className="fixed bottom-5 right-5 z-50 grid size-16 place-items-center rounded-full bg-[#ff7417] text-white shadow-[0_10px_28px_rgba(255,116,23,.35)] outline-none transition duration-300 hover:-translate-y-1 hover:scale-105 hover:bg-[#e85f08] focus-visible:ring-4 focus-visible:ring-orange-200 sm:bottom-8 sm:right-8 sm:size-20 lg:size-24"
            aria-label="Quick add meal"
          >
            <svg className="size-8 sm:size-10" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" />
            </svg>
          </button>
        </main>
