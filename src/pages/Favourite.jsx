...
export default function Favourite() {
  ...
  // Filter out recipes marked as favorites
  // Adjust 'item.isFavourite' based on your exact database key (e.g., item.liked)
  const favouriteRecipes = allRecipes?.filter((item) => item.isFavourite) || [];
          ...
          {favouriteRecipes.length === 0 ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {favouriteRecipes.map((recipe) => (
                ...
                >
                  {/* Decorative Fallback Image Vector */}
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <div className="mt-2 flex items-center gap-3 text-xs font-bold text-[#8a5c45]">
                      <span className="flex items-center gap-1">
                        <svg className="size-3.5 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                        Highly Rated
                      </span>
                      <span>·</span>
                    ...
                    <div className="mt-5 flex items-center justify-between border-t border-orange-50 pt-4">
                      <div>
                        ...
                        <p className="text-base font-black text-[#ff7417]">₦{Math.round(recipe.costEstimate || 0).toLocaleString()}</p>
                      </div>
