...
export default function ModalPlanner({ onClose, onPlanSubmit }) {
  const navigate = useNavigate()

  // Read existing plan from localStorage if available
  let savedPlanner = null
  try {
    savedPlanner = JSON.parse(localStorage.getItem("mealPlanned") || "null")
  } catch (error) {
    savedPlanner = null
  }

  const [weeklyBudget, setWeeklyBudget] = useState(savedPlanner?.budget || "")
  const [weeklyPeople, setWeeklyPeople] = useState(savedPlanner?.mealPeople || "")
  const [weeklyMeals, setWeeklyMeals] = useState(savedPlanner?.mealNumber || "")
  const [weeklyDiet, setWeeklyDiet] = useState(savedPlanner?.diet || "")
  ...
  function openPlannerRecipe() {
    ...
    // 1. Remove old generated plan so Planner.jsx generates a fresh set
    localStorage.removeItem("mealPlanned")

    // 2. Persist updated configuration options
    const plannerConfig = {
      budget: Number(weeklyBudget),
      mealPeople: Number(weeklyPeople),
      mealNumber: String(weeklyMeals),
      diet: weeklyDiet,
      plan: [] // Empty out old meals
    }
    localStorage.setItem("mealPlanned", JSON.stringify(plannerConfig))

    // 3. Navigate with search params
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[1, 2, 3, 4].map((num) => (
                <button
                  ...
                  data-selected={String(weeklyPeople) === String(num) ? "true" : "false"}
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  ...
                  data-selected={String(weeklyMeals) === String(option.num) ? "true" : "false"}
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  ...
                  data-selected={String(weeklyDiet) === String(option.value) ? "true" : "false"}
