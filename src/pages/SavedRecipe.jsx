...
function SavedRecipe() {
  ...
  console.log("savedRecipes in SavedRecipe.jsx:", savedRecipes);

  // Safe calculated values to prevent NaN / Infinity errors when list is empty
  const totalSaved = savedRecipes.length;
  
  const avgTime = totalSaved > 0 
    ? Math.round(savedRecipes.reduce((acc, recipe) => acc + (recipe.cookTime || 0), 0) / totalSaved) 
    : 0;

  const healthyCount = savedRecipes.filter((recipe) => recipe.healthy).length;

  const quickestTime = totalSaved > 0 
    ? Math.min(...savedRecipes.map((r) => r.cookTime || 0)) 
    : 0;

  const topRating = totalSaved > 0 
    ? Math.max(...savedRecipes.map((r) => r.rating || 0)) 
    : 0;

  const uniqueCategories = totalSaved > 0 
        ...
        <main className="relative min-h-[1038px] w-full max-w-7xl overflow-hidden bg-[radial-gradient(circle_at_18%_4%,rgba(255,127,28,.13),transparent_24%),linear-gradient(180deg,#fff8ef_0%,#fff4e8_46%,#fff7ef_100%)] px-[22px] pb-7 pt-[27px] max-[560px]:px-3.5 max-[560px]:pb-24">
          ...
          <section className="mt-[26px] grid grid-cols-3 gap-3 max-[560px]:grid-cols-1">
            <div className="min-h-[92px] rounded-[14px] border border-line bg-paper/85 p-[18px] shadow-card"> 
              ...
              <strong className="mt-[9px] block text-2xl font-extrabold tracking-[-.03em]">{totalSaved}</strong>
            </div>
            ...
            <div className="min-h-[92px] rounded-[14px] border border-line bg-paper/85 p-[18px] shadow-card">
              ...
              <strong className="mt-[9px] block text-2xl font-extrabold tracking-[-.03em] text-ember">{avgTime} min</strong>
            </div>
            ...
            <div className="min-h-[92px] rounded-[14px] border border-line bg-paper/85 p-[18px] shadow-card">
              ...
              <strong className="mt-[9px] block text-2xl font-extrabold tracking-[-.03em] text-[#38a136]">{healthyCount}</strong>
            </div>
          </section>
          ...
          <section className="mt-[22px] rounded-[14px] border border-line bg-paper/85 p-[18px] shadow-card">
            <div className="mb-[15px] flex items-center justify-between">
              ...
              <span className="text-xs font-black text-ember cursor-pointer">View All</span>
            </div>

            {totalSaved === 0 ? (
              <div className="py-12 text-center text-sm font-semibold text-clay">
                You haven't saved any recipes yet. Tap the bookmark icon on your dashboard to add some!
              </div>
            ) : (
              savedRecipes.map((recipe, index) => (
                <div key={recipe.id || index}>
                  <article className="mt-[13px] grid min-h-[105px] grid-cols-[112px_1fr_auto] items-center gap-[17px] rounded-xl border border-[#f1dfcd] bg-white/85 p-[10px_13px_10px_10px] shadow-soft max-[560px]:grid-cols-[92px_1fr]">
                    ...
                    <div>
                      <h3 className="mb-[7px] mt-0 text-base font-extrabold tracking-[-.015em] text-[#3a2119]">{recipe.name}</h3>
                      <div className="text-[11px] font-black text-ember">{recipe.category} • {recipe.cookTime} mins</div>
                      <div className="mt-2 flex gap-2 text-[11px] font-bold text-clay"><span>720 kcal</span><span>•</span><span>{recipe.servings} servings</span></div>
                    </div>
                    <div className="flex items-center gap-[9px] max-[560px]:col-span-2 max-[560px]:justify-end">
                      ...
                      {/* Interactive Bookmark Removal Button */}
                      <button 
                        onClick={() => toggleSaved && toggleSaved(recipe)} 
                        className="grid h-8 w-8 place-items-center rounded-[9px] border border-line bg-[#fffaf4] text-[#d95510]"
                        aria-label="Remove recipe from saved"
                      >
                        <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="#ff7419"><path d="M19 21l-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16Z" /></svg>
                      </button>
                    </div>
                  </article>
                </div>
          ...
          <section className="mt-[22px] rounded-[14px] border border-line bg-paper/85 p-[18px] shadow-card">
            <div className="grid grid-cols-4 gap-3 max-[560px]:grid-cols-1">
              <div className="flex items-center gap-[11px]">
                ...
                <span><small className="block text-[10px] font-extrabold text-clay">Quickest</small><strong className="mt-[3px] block text-sm font-extrabold text-[#3a2119]">{quickestTime} mins</strong></span>
              </div>
              <div className="flex items-center gap-[11px]">
                ...
                <span><small className="block text-[10px] font-extrabold text-clay">Top Rating</small><strong className="mt-[3px] block text-sm font-extrabold text-[#3a2119]">{topRating}</strong></span>
              </div>
              <div className="flex items-center gap-[11px]">
                ...
                <span><small className="block text-[10px] font-extrabold text-clay">Categories</small><strong className="mt-[3px] block text-sm font-extrabold text-[#3a2119]">{uniqueCategories} types</strong></span>
              </div>
              <div className="flex items-center gap-[11px]">
                ...
                <span><small className="block text-[10px] font-extrabold text-clay">This Week</small><strong className="mt-[3px] block text-sm font-extrabold text-[#3a2119]">{totalSaved} saved</strong></span>
              </div>
            </div>
          </section>
